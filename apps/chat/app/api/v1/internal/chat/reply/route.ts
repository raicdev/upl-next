...
export async function POST(req: Request) {
  ...
  const replyingTo = body.replyTo;
  const idToken = req.headers.get("Authorization");

  if (!content) {
    console.error(
      "[ERROR] Client Side Response Error: content is not set (/api/reply)",
      req.body
    );
    return NextResponse.json(
      {
  ...
  if (!replyingTo) {
    console.error(
      "[ERROR] Client Side Response Error: replyingTo is not set (/api/reply)",
      req.body
    );
    return NextResponse.json(
      {
        statusMessage: "Bad Request",
        errorCode: "required-options-not-set",
        errorMessage: "必要なオプション ('replyingTo'が設定されていません。)",
      },
      { status: 400 }
    );
  }
  ...
  if (!idToken) {
    console.error(
      "[ERROR] Client Side Response Error: idToken is not set (/api/reply)"
    );
