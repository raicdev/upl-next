  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant={"ghost"} className="p-2">
          <span className="inline-flex items-center justify-center">
            {modelDescriptions[model]?.type === "ChatGPT" && (
              <SiOpenai className="mr-2" />
            )}
            {modelDescriptions[model]?.type === "Gemini" && (
              <SiGooglegemini className="mr-2" />
            )}
            {modelDescriptions[model]?.type === "Claude" && (
              <SiClaude className="mr-2" />
            )}
            {modelDescriptions[model]?.type === "Grok" && (
              <SiX className="mr-2" />
            )}
            {modelDescriptions[model]?.type === "DeepSeek" && (
              <svg
                className="mr-2"
      ...
      <DropdownMenuContent className="w-64">
        <DropdownMenuLabel>リリースモデル</DropdownMenuLabel>
        <DropdownMenuGroup>
          {Object.keys(modelDescriptions).map(
            (modelKey) =>
              !modelDescriptions[modelKey]?.beta && (
                <DropdownMenuItem
                  key={modelKey}
                ...
                >
                  ...
                  <div className="flex items-center w-full justify-between">
                    ...
                    {modelDescriptions[modelKey]?.beta && <Badge>ベータ</Badge>}
        ...
        <DropdownMenuSeparator />
        <DropdownMenuLabel>試験的モデル</DropdownMenuLabel>
